...
public class SN {
    ...
    public void unfollows(int follower_page_id, int followed_page_id)
            throws SQLException {
        if (follower_page_id != followed_page_id) {
            String sql = "DELETE FROM follower WHERE page_ids=? AND page_idd=?";
            PreparedStatement stmt = theconnection.prepareStatement(sql);
            stmt.setInt(1, follower_page_id);
            stmt.setInt(2, followed_page_id);
            stmt.executeUpdate();
        }
    }
    ...
    public List<PageObject> getPendingFollowers(int page_id) throws SQLException {
        String sql = "SELECT page_ids FROM follower WHERE status=? AND page_idd=?";
        PreparedStatement stmt = theconnection.prepareStatement(sql);
        stmt.setString(1, FState.PENDING.toString());
        stmt.setInt(2, page_id);

        List<PageObject> pendingFollowers = new ArrayList<>();
        ResultSet rs = stmt.executeQuery();
        while (rs.next()) {
            PageObject p = getPage(rs.getInt("page_ids"));
            pendingFollowers.add(p);
        }

        return pendingFollowers;
    }

    public PageObject getPageByUserId(String userId) throws SQLException {
        String sql = "SELECT * FROM page WHERE user_id=?";
        PreparedStatement stmt = theconnection.prepareStatement(sql);
